...
@section('content')
    <div class="m-6">
        ...
        <form class="max-w-md mx-auto" method="POST" action="{{ route('store.update', $store->id) }}">
            @csrf
            @method('PUT')
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" name="edit_store_name" id="store_name" value="{{ $store->store_name }}"
                ...
                <label for="store_name"
                    ...
                    name</label>
                @error('edit_store_name')
                    <p class="text-sm text-red-500 mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="relative z-0 w-full mb-5 group">
                <input type="number" name="edit_latitude" id="latitude" step="0.000001" value="{{ $store->latitude }}"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " />
                <label for="latitude"
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="number" name="edit_longitude" id="longitude" step="0.000001" value="{{ $store->longitude }}"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " />
                <label for="longitude"
